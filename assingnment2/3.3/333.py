...
class AutoEncoder(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.LSTM(input_size=HIDDEN_STATE_SIZE, hidden_size=SEQUENCE_SIZE, num_layers=NUM_LAYERS,
                               batch_first=True)
        # self.predicter = nn.LSTM(input_size=1, hidden_size=1, num_layers=NUM_LAYERS, batch_first=True)
...
model = AutoEncoder()
critertion_recon = nn.MSELoss()
critertion_pred = nn.MSELoss()
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for stock in stocks:
            ...
            for i in range(0, data.shape[0], BATCH_SIZE):
                ...
                recon = model(seq)
                recon = recon.reshape(BATCH_SIZE, SEQUENCE_SIZE)
                loss_recon = critertion_recon(recon, seq)
                #loss_pred = critertion_pred(pred, expected_pred)
...
def test(stocks):
    ...
    for stock in stocks:
        ...
        loss = critertion_recon(recon, data)
        losses.append(loss)
